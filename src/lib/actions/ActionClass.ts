...
export class Action {
  ...
  private events: { [eventName: string]: (() => void)[] } = {};

  constructor({
    audioFile,
    element,
    text
  }: { audioFile?: string, text: string, element: SvelteComponent }) {
    this.text = text;
  ...
  addEventListener(eventName: string, callback: () => void): () => void {
    this.events[eventName] = eventName in this.events ? [...this.events[eventName], callback] : [callback];
    return () => {
