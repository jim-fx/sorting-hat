...
export default function createActions(anim: Animator): Action {
	...
	const rootAction = new Action({
		audioFile: 'teilnehmen.mp3',
		text: 'So so, du willst also an der Silvesterparty im Dungeon teilnehmen?'
	});
	...
	const name = new Action({
		audioFile: 'namen.mp3',
		text: 'Sehr schön, verstanden, jetzt bräuchte ich noch deinen Namen?',
		element: NameInput
	});
	...
	const dontStartQuiz = new Action({
		audioFile: 'unterhaltung.mp3',
		text: 'Das ist natürlich auch in Ordnung, damit du aber trotzdem nicht leer ausgehst habe ich hier etwas Unterhaltung für dich:',
		element: Video
	});

	const startQuiz = new Action({
		audioFile: 'exzellenter_gast.mp3',
		text: 'Naja, okay, wenn das sooo ist? Dann werden wir mal schauen... Verstanden, du siehst wie ein exzellenter Gast aus.'
	});

	const whichHouse = new Action({
		audioFile: 'welches_haus.mp3',
		text: 'Weisst du den schon zu welchem der vier Häuser du gehörst?'
	});
	...
	const randomHouse = new Action({
		audioFile: 'random_haus.mp3',
		text: 'Okay, um das herauszufinden bin ich ja hier. Dann werde ich mal  schauen, hmm, ich sehe Mut, aber auch Intelligenz, oh, und hier ist auch noch Empathie. Schwierige Entscheidung, aber ich denke:',
		element: RandomHouse
	});
	...
	const confidenceArrival = new Action({
		audioFile: 'sicherheit.mp3',
		text: 'Nun zu letzten Frage, auf einer Skala von 0 bis 10, wie sicher können wir mit deinem Erscheinen rechnen?',
		element: InputRange
	});
	...
	const finalAction = new Action({
		audioFile: 'final.mp3',
		text: 'So, das war auch schon alles. Wir freuen uns, dich am 31.12 willkommen zu heißen, und wünschen dir bis dahin eine schöne Weihnachtszeit.'
	});
