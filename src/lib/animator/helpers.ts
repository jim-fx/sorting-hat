...
export function bonesToObject(boneArray: Bone[]): { [key: string]: Bone | Bone[] } {
	const bones = {};
	...
	boneArray.forEach((bone) => {
		...
		if (name) {
			...
			if (memberName) {
				bones[groupName] =
					groupName in bones
						? { ...bones[groupName], ...{ [memberName]: bone } }
						: { [memberName]: bone };
				bones[groupName + 'Array'] =
					groupName + 'Array' in bones ? [...bones[groupName + 'Array'], bone] : [bone];
			}
		}
	});
...
function _boneObjectToRotations(
	boneObject: { [key: string]: Bone | Bone[] } | Bone | Bone[],
	cache = { bones: {} },
	depth = 0
) {
...
export function applyRotationToBones({
	_all,
	_bones
}: {
	_all: BoneRotation[];
	_bones: { [key: string]: Bone };
}): void {
	_all.forEach((b) => {
		if (!b.dirty) return;
